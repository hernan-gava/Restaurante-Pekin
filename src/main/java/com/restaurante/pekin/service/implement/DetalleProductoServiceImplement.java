...
public class DetalleProductoServiceImplement implements DetalleProductoService {
  ...
  public void guardarDetallePedidoCliente(DetalleProducto detalleProducto) throws DetalleMenuException {
    ...
    try {
      if (detalleProducto.getNombre().trim().isEmpty() || detalleProducto.getCalle().trim().isEmpty() || detalleProducto.getFechahoracaptura().trim().isEmpty() || detalleProducto.getLocalidad().trim().isEmpty() || detalleProducto.getPedido().trim().isEmpty() || detalleProducto.getReferencia().trim().isEmpty() || detalleProducto.getTelefono().trim().isEmpty() || detalleProducto.getTotalpago() < 0 || detalleProducto.getActivo().trim().isEmpty()) {
      ...
      mensaje.append("NO SE PUDO GUARDAR DETALLE DEL PEDIDO DEL CLIENTE ERROR EN EL CATCH").append(ex.getMessage());
